...
The Phase 0 foundation, deterministic testkit, generated protocol consumers, golden contract tests, bootstrap policy, and local CI gates are implemented. The Phase 1 audio primitives and composed `relay-audio` path are also implemented and locally gated: bounded realtime rings, scheduled-playout clock recovery, RTP sequence reordering, fixed/adaptive Rubato resampling, a safe libopus boundary, loopback, 60-second media, finite drain, and the 12-virtual-hour soak. Phase 0 still requires a tracked Git baseline and hosted Linux/Windows/macOS CI; Phase 1 still requires a final callback-safety audit and locked workspace/platform runs. Native unpaid Connect and Stream work on localhost and the home LAN through `relay-session`, the CLIs, and the Truce plugin. **LAN is 5 ms uncompressed PCM** (no Opus). Named-session listen is live at [https://relay.matari-audio.com](https://relay.matari-audio.com). Download: [matari-audio.com/relay](https://matari-audio.com/relay). Test recipes: [`apps/plugin/README.md`](apps/plugin/README.md). Billing and hosted three-OS CI remain open. Plugin→browser listen is P2P through `relay-transport` + libdatachannel: sendonly Opus audio track, Cloudflare STUN, plus Cloudflare Realtime TURN when the room has `TURN_KEY_ID` and `TURN_KEY_TOKEN` set (`wrangler secret put <name>` in `apps/relay-web`); unset means STUN only. Both the browser and the plugin fetch `GET /api/ice?room=<slug>` for per-session relay credentials, and libdatachannel drops relay URLs its ICE backend cannot dial (juice does `turn:` over UDP only; libnice adds TCP and TLS). RELAY is free to run and free to use: a relay grant is spent only on a room whose host is connected, only when a listener actually builds a peer connection, and only while the month's grant budget holds. The default budget of 4,000 grants caps worst-case egress at 921 GB — inside Cloudflare's 1,000 GB free tier — so the service degrades to STUN rather than ever billing the operator. Raise it with the `ICE_GRANTS_PER_MONTH` var once real usage justifies it.
